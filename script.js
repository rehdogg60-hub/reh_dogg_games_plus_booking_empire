...
const audio = {
  enabled: storageGet("bookingEmpireSound", "on") !== "off",
  musicActive: false,
  unlocked: false,
  music: new Audio("sounds/menu_theme.mp3"),
  click: new Audio("sounds/click.mp3"),
  crowd: new Audio("sounds/crowd.mp3"),
  splashSting: new Audio("sounds/splash_sting.mp3"),
...
function toggleSound() {
  ...
  storageSet("bookingEmpireSound", audio.enabled ? "on" : "off");
  updateSoundButton();
  if (storageGet(SAVE_KEY)) saveGame(currentScreen);
}
...
const loadingTips = [
  "Hot feuds increase PPV revenue.",
  "Low morale may cause wrestlers to leave.",
  "Title feuds raise stakes, but bad title feuds hurt prestige.",
  "Rival promotions chase unhappy stars.",
  "Strong promos can turn a cold rivalry into money."
];
...
function startReleasePresentation() {
  ...
  const timer = window.setInterval(() => {
    progress = Math.min(100, progress + Math.ceil(7 + Math.random() * 12));
    if (loadingBar) loadingBar.style.width = `${progress}%`;
    if (loadingPercent) loadingPercent.textContent = `${progress}%`;
    if (progress >= (tipIndex + 1) * 22 && loadingTip) {
      tipIndex = Math.min(loadingTips.length - 1, tipIndex + 1);
...
function saveGame(screenName = currentScreen) {
  ...
  try {
    if (!storageSet(SAVE_KEY, JSON.stringify(data))) throw new Error("localStorage unavailable");
...
function updateContinueButton() {
  ...
  if (!button) return;
  button.classList.toggle("has-save", Boolean(storageGet(SAVE_KEY)));
}
...
function maybeShowTutorialPrompt() {
  ...
  if (!prompt || storageGet(TUTORIAL_KEY) === "yes") return;
  prompt.hidden = false;
}
...
function closeTutorialPrompt(markComplete = false) {
  ...
  if (markComplete) storageSet(TUTORIAL_KEY, "yes");
}
...
document.getElementById("creditsBtn").addEventListener("click", () => show("credits"));
...
document.getElementById("backTitleFromCreditsBtn").addEventListener("click", () => show("title"));
...
document.addEventListener("pointerdown", event => {
  ...
  const startupActive = document.body.classList.contains("startup-active");
  if (startupActive) {
    unlockAudio();
    playEffect(audio.splashSting, 0.58);
  }
