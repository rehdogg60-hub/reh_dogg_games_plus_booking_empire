...
function updateMoney() {
  ...
  if (contractsMoney) contractsMoney.textContent = moneyText(money);
...
function renderLegends() {
  ...
  if (!grid) return;
  grid.innerHTML = hallOfLegends.length
    ? hallOfLegends.slice().reverse().map(legend => `
      <article class="news-card major-news">
        <span>${legend.careerStage || "Legend"}</span>
        <h3>${legend.name}</h3>
        <p>${legend.note}</p>
        <small>Popularity ${legend.popularity} | Momentum ${legend.momentum}</small>
      </article>
    `).join("")
    : `<article class="news-card"><span>Hall Empty</span><h3>No RDGP legends have retired yet.</h3><p>Build stars, protect veterans, and give them a memorable final run.</p></article>`;
}
...
function runWeek() {
  ...
  result.commentary = pickCommentary(netProfit, result.heatChange, activeFeud.risk, result.rating, result.crowd.level);
...
function retireWrestler(index, reason) {
  ...
  addNews(`${wrestler.name} ${reason}.`, "RDGP company news", "major");
}
